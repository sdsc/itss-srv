...
<html>
    ...
    <script>
        ...
        function getEstimate(id, price, dest)
        {
            ...
            /*var vm1 = parseFloat(document.getElementById('vm-1-qty').value);
            var vm1cpu = parseFloat(document.getElementById('vm-1-cpu').value);
            var vm1mem = parseFloat(document.getElementById('vm-1-mem').value);
            var vm1storage = parseFloat(document.getElementById('vm-1-storage').value);
            
            var vm2 = parseFloat(document.getElementById('vm-2-qty').value);
            var vm2cpu = parseFloat(document.getElementById('vm-2-cpu').value);
            var vm2mem = parseFloat(document.getElementById('vm-2-mem').value);
            var vm2storage = parseFloat(document.getElementById('vm-2-storage').value);
            
            var unixvm1 = parseFloat(document.getElementById('unix-vm-1-qty').value);
            var unixvm1cpu = parseFloat(document.getElementById('unix-vm-1-cpu').value);
            var unixvm1mem = parseFloat(document.getElementById('unix-vm-1-mem').value);
            var unixvm1storage = parseFloat(document.getElementById('unix-vm-1-storage').value); */
            
            /* do the calculations */
            /*var vm1sub = vm1 * VM_PRICE;
            var vm1cpusub = vm1cpu * VM_CPU;
            var vm1memsub = vm1mem * VM_MEM;
            var vm1storagesub = vm1storage * VM1_STORAGE; */
            
            /* input the values */
            /*document.getElementById('vm-1-sub').value = "$" +  vm1sub.toFixed(2);
            document.getElementById('vm-1-cpu-sub').value = "$" + vm1cpusub.toFixed(2);
            document.getElementById('vm-1-mem-sub').value = "$" + vm1memsub.toFixed(2);
            document.getElementById('vm-1-storage-sub').value = "$" + vm1storagesub.toFixed(2); */
        }
    </script>
    </head>
    <body>
       <table>
           ...
           <tr class="vm-1">
               ...
               <td>$69.75/month</td>
               <td><input type="text" class="subtotal" id="vm-1-sub" readonly="true"></td>
           </tr>
           <tr class="vm-1">
